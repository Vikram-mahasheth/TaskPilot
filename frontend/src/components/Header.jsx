...
// 1. The NotificationBell is now a standalone, stable component defined OUTSIDE the Header.
// This prevents it from being re-created on every render, which solves the race condition.
const NotificationBell = forwardRef((props, ref) => {
    ...
    // This handler now ONLY closes the menu and marks the notification as read.
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setIsOpen(prev => !prev)} className="relative p-2 rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700">
                <Bell size={20} />
                {unreadCount > 0 && (
                ...
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-md shadow-lg overflow-hidden z-20 border dark:border-gray-700">
                    ...
                    <ul className="divide-y divide-gray-200 dark:divide-gray-700 max-h-96 overflow-y-auto">
                        {notifications.length > 0 ? notifications.map(n => (
                            <Link key={n._id} to={n.link || '/'} onClick={() => handleNotificationRead(n)}>
                                <li className={`px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer ${!n.read ? 'bg-indigo-50 dark:bg-indigo-900/50' : ''}`}>
